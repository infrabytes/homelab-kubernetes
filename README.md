...
| Sealed Secrets               | One-way encrypted Secrets committed to git (Bitnami chart, controller `sealed-secrets-controller` in namespace `sealed-secrets`); complements SOPS for app-level secrets |
...
platform/           ArgoCD-managed cluster-level resources (network, issuer,
  ...
  helm-charts/      one parent ArgoCD app (app-of-apps) for the Helm chart
                    ...
                    grafana-cloud, longhorn, prometheus-operator-crds, sealed-secrets,
                    spegel, vcluster, argocd-diff-preview, gha-runner-scale-set,
