...
**DNS visibility** (`dns-visibility.yaml`): a cluster-wide egress policy that
names kube-dns with `toPorts[].rules.dns` — the form Cilium needs to route DNS
through its proxy and emit the `hubble_dns_*` metrics behind the Hubble DNS
dashboard (a bare `toFQDNs` rule is realized but does not engage the proxy,
measured 2026-09-12). `toEntities: cluster/world` keeps egress exactly as open
as it is today, and ingress policies are untouched.
