...
variable "grafana_cloud_adaptive_metrics_token" {
  description = "Grafana Cloud access-policy token with the adaptive-metrics-config:read and adaptive-metrics-recommendations:write scopes. Held in the shared SOPS secrets file."
  type        = string
  sensitive   = true
}
